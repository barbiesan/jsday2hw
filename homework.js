...
let addThree = function createAdder(add){
    function inner(num2){
        return num2 += add
    }
    return inner
...
movieName('Indiana Jones and the Dark Knight').then(s => console.log(` Indiana Jones and the Dark Knight directed by Christopher Spielberg.${m}`))
movieName('cha').then(s => console.log(`*Warning* ET cannot be accessed because it it too short.${m}`)).catch(err => console.warn(e))
